...
def get_largest_area_part2(coords):
    ...
    print(f"Total red/green tiles: {len(red_green_set)}")
...
# Debug: check specific rectangle (9,5) to (2,3)
red_green = precompute_red_green_tiles(data)
min_x = min(2, 9)
max_x = max(2, 9)
min_y = min(5, 3)
max_y = max(5, 3)
print(f"\nChecking rectangle from (9,5) to (2,3)")
...
for x in range(min_x, max_x + 1):
    for y in range(min_y, max_y + 1):
        if (x, y) in red_green:
            ...
            print(f"  ({x},{y}) is NOT red/green")
print(f"Total tiles in rectangle: {(max_x-min_x+1)*(max_y-min_y+1)}")
print(f"Red/green tiles: {count}")

result = get_largest_area_part2(data)
